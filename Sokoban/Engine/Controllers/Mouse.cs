...
public static partial class Controller
{
  ...
  private static Action<double> HandleState(MouseButton wanted, Action<float> action, Func<bool>? predicate = null) =>
    (predicate is null).Or<Action<double>>(
      dt => Mouse.IsButtonPressed(wanted).Then(action, (float)dt),
      dt => (predicate!() && Mouse.IsButtonPressed(wanted)).Then(action, (float)dt)
    );
}
}